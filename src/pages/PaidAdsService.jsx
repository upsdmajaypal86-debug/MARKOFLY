...
import {
  ...
  paidAdsProcess,
  paidAdsTechStack,
} from '../content/paidAdsService';

const highlightIcons = [Target, Users, Zap];
const capabilityIcons = [Search, Megaphone, Target, LineChart, MousePointer2, CreditCard];
    ...
    >
        ...
        <div className="container mx-auto grid max-w-7xl gap-14 px-6 lg:grid-cols-[1.05fr_0.95fr] lg:items-center">
          <div>
            <nav aria-label="Breadcrumb" className="mb-8 flex flex-wrap items-center gap-2 text-sm text-slate-500">
              ...
              <span>/</span>
              <span className="font-medium text-slate-700">Paid Advertising & PPC</span>
            </nav>
            ...
            <h1 className="max-w-3xl text-4xl font-extrabold leading-tight text-slate-950 md:text-6xl">
              Precision-targeted paid advertising built to drive immediate value and long-term ROI.
            </h1>

            <p className="mt-6 max-w-2xl text-lg leading-8 text-slate-600 md:text-xl">
              We manage high-performance Google, Meta, and LinkedIn campaigns that capture intent, build demand, and 
              convert your ideal customers at the lowest possible cost.
            </p>

            <div className="mt-8 flex flex-wrap gap-3">
              {paidAdsHeroPills.map((pill) => (
                <span
                  key={pill}
            ...
            <GlassCard className="relative overflow-hidden rounded-[2.5rem] border-white/70 bg-white/80 p-0 shadow-[0_28px_90px_rgba(59,130,246,0.18)]">
              <div className="bg-slate-950 px-6 py-6 text-slate-100">
                ...
                <div className="rounded-[1.75rem] border border-white/10 bg-white/5 p-5 font-mono text-sm leading-7 text-slate-300">
                  <p>{'campaign = {'}</p>
                  <p className="pl-4 text-sky-300">target: 'high-intent',</p>
                  <p className="pl-4 text-emerald-300">bidding: 'roi-optimized',</p>
                  <p className="pl-4 text-violet-300">tracking: 'server-side-capi',</p>
                  <p className="pl-4 text-amber-300">scaling: 'data-validated'</p>
                  <p>{'}'}</p>
                </div>
              </div>

              <div className="grid gap-4 p-6 md:grid-cols-3">
                {paidAdsMetrics.map((metric) => (
      ...
      <Section className="pt-0">
        <div className="mx-auto mb-14 max-w-3xl text-center">
          ...
          <h2 className="text-3xl font-bold text-slate-950 md:text-5xl">
            Success in paid advertising is a game of data, not just budget.
          </h2>
          <p className="mt-5 text-lg leading-8 text-slate-600">
            Our PPC approach focuses on the metrics that matter—customer acquisition cost and total return on investment—so 
            you can scale with confidence.
          </p>
        </div>

        <div className="grid gap-6 md:grid-cols-3">
          {paidAdsHighlights.map((item, index) => {
      ...
      <Section className="bg-slate-50">
        <div className="grid gap-12 lg:grid-cols-[0.85fr_1.15fr] lg:items-start">
          <div>
            ...
            <h2 className="text-3xl font-bold text-slate-950 md:text-5xl">
              Performance marketing for brands that need measurable growth.
            </h2>
            <p className="mt-5 text-lg leading-8 text-slate-600">
              Whether you need to capture demand on search, build awareness on social, or remarket to existing visitors, 
              we shape the channel strategy and ad creative around your business goals.
            </p>
          ...
          <div className="grid gap-6 md:grid-cols-2">
            {paidAdsCapabilities.map((item, index) => {
      ...
      <Section>
        <div className="mx-auto mb-14 max-w-3xl text-center">
          ...
          <h2 className="text-3xl font-bold text-slate-950 md:text-5xl">A clear path from spend to revenue.</h2>
          <p className="mt-5 text-lg leading-8 text-slate-600">
            We follow a structured PPC workflow that focuses on data collection, aggressive optimization, and strategic scaling.
          </p>
        </div>

        <div className="grid gap-6 md:grid-cols-2 xl:grid-cols-4">
          {paidAdsProcess.map((item) => (
      ...
      <Section className="bg-slate-950 text-white">
        <div className="grid gap-10 lg:grid-cols-[0.9fr_1.1fr] lg:items-center">
          <div>
            <p className="mb-4 text-sm font-bold uppercase tracking-[0.25em] text-sky-300">Platforms & Foundations</p>
            <h2 className="text-3xl font-bold md:text-5xl">
              Proven tools and high-performance tracking systems.
            </h2>
            <p className="mt-5 text-lg leading-8 text-slate-300">
              We leverage modern tracking frameworks and agency-level analytics to ensure zero "lost data" and maximum visibility into campaign impact.
            </p>

            <div className="mt-8 flex flex-wrap gap-3">
              {paidAdsTechStack.map((item) => (
                <span
                  key={item}
          ...
          <div className="grid gap-5 md:grid-cols-2">
            {[
              {
                title: 'Conversion-first tracking',
                description: 'Server-side GTM and CAPI implementations to ensure accurate data in a post-cookie web.',
              },
              {
                title: 'ROI-driven management',
                description: 'Every bidding strategy and budget shift is calculated to improve your bottom-line revenue.',
              },
              {
                title: 'High-performing creative',
                description: 'Data-driven copy and creative direction that stops the scroll and drives the click.',
              },
              {
                title: 'Clear attribution mapping',
                description: 'Understand exactly which channel, ad, and audience is driving your highest-value customers.',
              },
            ].map((item) => (
      ...
      <Section className="bg-slate-50">
        <div className="mx-auto mb-14 max-w-3xl text-center">
          ...
          <h2 className="text-3xl font-bold text-slate-950 md:text-5xl">Questions brands ask before they spend.</h2>
          <p className="mt-5 text-lg leading-8 text-slate-600">
            Find answers to common questions about budget, platform choice, and what to expect in the first 30 days.
          </p>
        </div>

        <div className="grid gap-6 md:grid-cols-2">
          {paidAdsFaqs.map((item) => (
      ...
      <CTASection
        ...
        description="Tell us about your conversion goals, and we will help perform a quick audit of your current ad account performance."
